...
#define ITERS 10000000 // Number of iterations per thread

/* A SHARED global counter. Both threads will modify this. */
long counter = 0;

void *increment(void *arg) {
    (void)arg         /* unused — pthread signature requires it */;// Avoid unused parameter warning
    for (long i = 0; i < ITERS; i++) {
        long tmp = counter + 1;     /* three machine instructions in disguise */
        for(int i=0; i<10000; i++); /* add some delay to increase the chance of a race condition */
        tmp = tmp +1;
        counter = tmp;
    }
    return NULL;
...
int main(void) {
    ...
    long expected = 2L * ITERS;// The expected final value if there were
